...
if (env.train):
    ...
    for epoch in range(1, number_epochs):
        ...
        print(" - Best Total loss: {:.6f} ".format(best_total_loss))
        print(" - Porcentaje de reducción {:02f}".format(((best_total_loss-loss)/best_total_loss)*100))
        print(" - Cont: {:.0f} ".format(cont_epoch_wait))
        
        #EARLY STOPPING
        
        #Compruebo la pérdia obtenida y la comparo con la mejor obtenida
        
        if early_stopping:
            
              #Compruebo si se han reducido las pérdidas un porcentaje 
              #mayor que el porcentaje seleccionado
              
              
            if (loss >= best_total_loss):  
                #La pérdidas no han mejorado
                cont_epoch_wait +=1
                
            else:    
                
                #La pérdidas han mejorado, compruebo si lo han hecho más de un porcentaje determinado
                if (((best_total_loss-loss)/best_total_loss)*100)> reduction_percent:
